...
# デプロイ時には使われないためあえて外して記載している。
urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT) # 画像表示のためのルーティング。staticという関数によって、requestされたURLと第一引数のMEDIA_URLが同じならその画像を返す。document_rootは画像の保存場所